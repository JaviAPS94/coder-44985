...
const login = async (req, res) => {
    //Implementar un contador de intentos de sesión, primero hay que identificar la IP del usuario que esta haciendo el request, para que en el caso de que el contador sea mayor a 3 vamos a bloquear los request desde la IP mencionada (black list) A07
    ...
    //La clave para firmar nuestro jwt debe estar como variable de ambiente A05
    const token = jwt.sign(user.toObject(),'C0DerSecret',{expiresIn:30});
    //El token expira en 30 segundos pero puede seguir navegando porque la cookie no tiene tiempo de expiración A01
